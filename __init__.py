...
def scatter_plot_1(particles, **kwargs):
    ...
    if "ax" not in kwargs:
        fig, ax = plt.subplots()
        ax.set_title("Particule distribution, trapping particule")
        ax.set_xlabel("x")
        ax.set_ylabel("Vx")
        ax.set_xlim(0, 6.4)
    ...
    for pop in particles:
        ...
        im = ax.scatter(x_, v_, s=0.0001, c=colors[pop])
    return x_, v_

def scatter_plot_2(particles, **kwargs):
    ...
    if "ax" not in kwargs:
        ...
        ax.set_xlabel("x")
        ax.set_ylabel("Vx")
        ax.set_title("Particule distribution")
        ax.set_xlim(0, 32)
    else:
        ax = kwargs["ax"]
        fig = ax.figure
...
def dist_plot(self, **kwargs):
    ...
    elif plot_type == "scatter_1":
        return final, sp1(final, **kwargs)
    elif plot_type == "scatter_2":
        return final, sp2(final, **kwargs)